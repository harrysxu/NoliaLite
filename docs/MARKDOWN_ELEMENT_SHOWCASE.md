...
点击任意标题后应进入该标题的局部 Markdown 编辑态。把本行开头从 `######` 改为 `###`，提交后应立即变成三级标题；删除全部 `#` 后应变为普通段落。

## 行内格式与包裹字符

基础格式包括 **~~粗体文字~~**、*斜体文字*、~~删除线文字~~、`inlineCode()` 和 [相对链接](./UI_UX_SPEC.md)。

组合格式包括 ***粗斜体***、**粗体中的 *斜体***、~~删除线中的 **粗体**~~，以及 **包含 `inlineCode()` 的粗体**。点击具体格式后，应只把当前元素恢复成带 `**`、`*`、`~~` 或反引号的局部源码，其余段落继续保持渲染态。

包裹符号可直接修改：把 `**粗体文字**` 改成 `*粗体文字*` 后应成为斜体；把 `~~删除线文字~~` 的定界符删除后应成为普通文字。按 `Escape` 应取消本次修改并返回排版视图。
...
下面的短表不应被生硬地平均拉伸；文字、数值与状态应容易横向扫描。
